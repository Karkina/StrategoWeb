...
function Game({ lobbyId, player, socket }) {
    ...
    socket.on('phaseUpdate', (newPhase) => {
        ...
        if (newPhase === 'playing') {
            setTurnTimeLeft(TURN_DURATION);
             ...
             if (timerIntervalRef.current) clearInterval(timerIntervalRef.current); // Clear timer if not playing
        }
    });
  ...
  const isValidPlacement = useCallback((x, y) => {
    // Check if cell is occupied or impassable terrain
    if (board[x][y] || terrainTypes[terrain[x][y]].impassable) {
      return false;
    }
    // Player 1 places in rows 0, 1, 2
    if (player === 1 && x >= 0 && x <= 2) return true;
    // Player 2 places in rows 4, 5, 6
    if (player === 2 && x >= 4 && x <= 6) return true;
    // Otherwise, invalid
    return false;
    ...
    <div className="game-container">
      ...
      {phase === 'placement' && (
        <div className="placement-controls">
          ...
          <p>Drag your pieces onto the highlighted starting rows. Right-click a piece type for info.</p>
          ...
          <button onClick={handleReady} disabled={readyPlayers.includes(player)}>
             {readyPlayers.includes(player) ? 'Waiting for Opponent...' : 'Ready'}
          </button>
          <p className="ready-players">Ready Players: {readyPlayers.length} / 2</p>
        </div>
      )}
