...
function App() {
  ...
  useEffect(() => {
    ...
    socket.on('assignPlayer', (playerNumber) => setPlayer(playerNumber));
    socket.on('phaseUpdate', (newPhase) => setPhase(newPhase));
  ...
  // Handle dragging a piece from the reserve
  const handleDragStartPiece = (e, type) => {
    e.dataTransfer.setData('pieceType', type);
  };

  // Handle dragging a piece from the board (during playing phase)
  ...
  // Unified drop handler for both phases
  const handleDrop = (e, toX, toY) => {
    ...
    if (phase === 'placement' && pieceType) {
      if (isValidPlacement(toX, toY) && piecesLeft[pieceType] > 0 && terrain[toX][toY] !== 1) {
        socket.emit('placePiece', { x: toX, y: toY, type: pieceType, player });
        setPiecesLeft((prev) => ({ ...prev, [pieceType]: prev[pieceType] - 1 }));
      }
      ...
      if (turn === player && terrain[toX][toY] !== 1) {
        socket.emit('move', { fromX: fromXInt, fromY: fromYInt, toX, toY, player });
      }
    }
  };
  ...
  // Highlight valid drop zones during placement
      ...
      <div className="board">
        {terrain.map((row, x) =>
          row.map((terrainType, y) => (
            <div
              key={`${x}-${y}`}
              className={`cell terrain-${terrainType} ${
